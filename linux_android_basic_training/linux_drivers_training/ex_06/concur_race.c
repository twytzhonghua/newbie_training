...
ssize_t misc_write(struct file *filp, char __user *buf, size_t count, loff_t *offset)
{
	...
	*/
	...
	/*
	*cpu's speed is so fast, so I add some info to print in order to waste time
	*/
	num = 1000000;
	t1 = jiffies;//current count
	while(num)
	{	
		
		num--;
		printk("print some info to waste time\n");
	}
	t2 = jiffies;//after while loop, current count;
	/*
	*because kernel can't do floating point airthmetic, I just can get counts.
	*/
	printk("while loop costs %ld conuts\n",t2-t1);
//	t2 = (t2-t1)/HZ;	
//	printk("while loop costs %d seconds\n",t2);
	
	
	
	
	/*
	*test ssleep, use ssleep(), cpu can do other task, but udelay() can't,
	*udelay() is also used to wait for hardware, because some hardware can't keep pace with cpu.
	*/
	ssleep(8);
	//udelay(8);
